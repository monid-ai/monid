...
pools the account meters (`enrich`, `search`, `enrich_company`,
`search_company` — PDL's own `x-call-credits-type` values), NO
...
- **THEN** `pdl#v5/person/enrich` carries credits `{enrich}`, `v5/person/search` `{search}`, `v5/company/enrich` `{enrich_company}`, `v5/company/search` `{search_company}` — each doc narrowed to the one pool its model consumes
