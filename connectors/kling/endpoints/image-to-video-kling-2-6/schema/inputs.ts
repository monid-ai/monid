...
/**
 * `POST /image-to-video/kling-2.6` body — the faithful vendor mirror (D25).
 * Live docs https://kling.ai/document-api/api/video/2-6/image-to-video, read
 * 2026-09-16: prompt max 2500 (v1 said 3072), duration 5 | 10, no 4K. The
 * `voice` content type (an account-level custom-voice asset — and the
 * trigger for Kling's separate "voice control" rate) and `options` are
 * deliberately absent (v1 scope).
 */
export const zImageToVideoKling26Body = z.object({
    contents: z
        .array(
