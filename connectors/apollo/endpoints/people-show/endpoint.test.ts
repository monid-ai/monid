...
const ID = "apollo#people/show";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = { pathParams: { id: "PERSON1" } };
...
Deno.test(`${ID} happy (synthetic): one record bills one credit`, async () => {
    ...
    // the placeholder email is relayed as Apollo sends it (meta.notes)
    assertEquals(
        (result.output as { person: { email: string } }).person.email,
        "email_not_unlocked@domain.com",
    );
});

Deno.test(`${ID} provider error (synthetic 422, unknown person): zero usage`, async () => {
    const unit = await testSealedUnit(ID);
...
Deno.test({
    ...
    fn: async () => {
        ...
        const result = await runEndpoint({
            unit,
            // the person id from the People Enrichment reference example
            input: { pathParams: { id: "672c91d4a7be42000184f2c9" } },
            mode: "live",
        });
        assertEquals(
        ...
        assertEquals(
            typeof (result.output as Record<string, unknown>).person,
            "object",
            JSON.stringify(result.output),
        );
