...
/**
 * Lighthouse Audits — `GET /v3/on_page/lighthouse/audits` (v1
 * `/onpage/lighthouse-audits`). Free dictionary lookup (design D7).
 */
export default defineEndpoint({
    meta: {
        displayName: "Lighthouse Audits",
        summary: "List the audit ids a Lighthouse run can be limited to.",
        description:
            "Free lookup. Returns the Lighthouse audit ids grouped by " +
            "category, for the audits parameter of the lighthouse " +
            "endpoint. To use these values in /onpage/lighthouse, call " +
            "dataforseo#onpage/lighthouse (the endpoint this list is " +
            "for).",
        docsUrl: "https://docs.dataforseo.com/v3/on_page/lighthouse/audits/",
        categories: ["seo"],
    },
    endpoint: "/onpage/lighthouse-audits",
    request: { method: "GET", path: "/v3/on_page/lighthouse/audits" },
