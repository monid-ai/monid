...
import { zAppStoreAppInfoBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * App Store App Details — `POST /v3/app_data/apple/app_info/task_post` (v1
 * `/app-store/app-info`). Flat: $0.0012 per call (design D4). Queued
 * upstream: task_post at high priority, task_get polled (design D6).
 */
export default defineEndpoint({
    meta: {
        displayName: "App Store App Details",
        summary: "Fetch an App Store app page: rating, version, price, " +
            "developer.",
        description:
            "App Store app details by app_id. Returns title, developer, " +
            "category, description, rating and rating distribution, " +
            "votes, price, in-app purchases, version and release notes, " +
            "size, age rating, screenshots, and languages. Suited for app " +
            "profiles and competitor tracking.",
        docsUrl:
            "https://docs.dataforseo.com/v3/app_data/apple/app_info/task_post/",
        categories: ["app-stores"],
        notes: [
            "Queued at high priority (about a minute).",
        ],
    },
    endpoint: "/app-store/app-info",
    request: { method: "POST", path: "/v3/app_data/apple/app_info/task_post" },
    input: { schema: { body: zAppStoreAppInfoBody } },
