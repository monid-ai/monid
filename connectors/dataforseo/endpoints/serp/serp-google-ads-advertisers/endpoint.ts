...
import { zSerpGoogleAdsAdvertisersBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Google Ads Advertisers — `POST /v3/serp/google/ads_advertisers/task_post`
 * (v1 `/serp/google-ads-advertisers`). Flat: $0.0012 per call (design D4).
 * Queued upstream: task_post at high priority, task_get polled (design D6).
 */
export default defineEndpoint({
    meta: {
        displayName: "Google Ads Advertisers",
        summary:
            "Look up advertisers in the Google Ads Transparency Center by " +
            "name.",
        description:
            "Advertiser search in Google's Ads Transparency Center for a " +
            "keyword (advertiser or domain name). Returns advertisers " +
            "with advertiser_id, name, verified status, location, and " +
            "domains. Pass the advertiser_id to google-ads-search to list " +
            "their ads. Suited for competitor ad research. To find the " +
    ...
    endpoint: "/serp/google-ads-advertisers",
    request: {
        method: "POST",
        path: "/v3/serp/google/ads_advertisers/task_post",
    },
    input: { schema: { body: zSerpGoogleAdsAdvertisersBody } },
