...
const ID = "mrscraper#nordstrom/product";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = {
    body: {
        "url": "https://www.nordstrom.com/s/ultra-soft-zip-jacket/8036333",
    },
};

Deno.test(`${ID} happy (synthetic): 93 tokens, the caller gets the inner data`, async () => {
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    for (
        const bad of [
            ...
            {
                "url":
                    "https://nordstrom.attacker.example/s/ultra-soft-zip-jacket/8036333",
            },
            {
                "url":
                    "https://www.nordstromrack.com/s/ultra-soft-zip-jacket/8036333",
            },
        ]
    ) {
    ...
    const twin = await run({
        "url": "https://www.nordstrom.ca/s/ultra-soft-zip-jacket/8036333",
    });
