...
const ID = "mrscraper#lazada/category";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = {
    body: { "url": "https://www.lazada.sg/catalog/?q=earbuds" },
};
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    for (
        const bad of [
            { "url": "https://www.shopee.sg/catalog/?q=earbuds" },
            { "url": "https://lazada.attacker.example/catalog/?q=earbuds" },
            { "url": "https://www.lazada.sg/catalog/?q=earbuds", "page": 2 },
        ]
    ) {
    ...
    const twin = await run({
        "url": "https://www.lazada.com.my/catalog/?q=earbuds",
    });
