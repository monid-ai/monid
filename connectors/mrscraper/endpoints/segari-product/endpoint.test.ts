...
const ID = "mrscraper#segari/product";
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    for (
        const bad of [
            { "url": "https://segari.id/c/telur" },
            {
                "url":
                    "https://segari.attacker.example/p/telur-ayam-kampung-curah",
            },
            { "url": "https://www.sayurbox.com/p/telur-ayam-kampung-curah" },
        ]
    ) {
    ...
    const twin = await run({
        "url": "https://www.segari.id/p/telur-ayam-kampung-curah",
    });
