...
const ID = "mrscraper#hotels-com/reviews";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = {
    body: { "url": "https://th.hotels.com/en/ho338863/mitsui-garden-hotel/" },
};

Deno.test(`${ID} happy (synthetic): 24 tokens, the caller gets the inner data`, async () => {
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    for (
        const bad of [
            {
                "url":
                    "https://www.expedia.com/en/ho338863/mitsui-garden-hotel/",
            },
            {
                "url":
                    "https://hotels.attacker.example/en/ho338863/mitsui-garden-hotel/",
            },
            {
    ...
    const twin = await run({
        "url": "https://www.hotels.com/ho338863/mitsui-garden-hotel/",
    });
