...
const ID = "mrscraper#youtube/comments";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = { body: { videoId: "HeyIXwZyR8Y" } };
...
Deno.test(`${ID}: the compiled schema gates the input`, async () => {
    ...
    for (
        const bad of [
            { videoId: "https://www.youtube.com/watch?v=HeyIXwZyR8Y" },
            { videoId: "abc" },
            {},
        ]
    ) {
    ...
    const twin = await run({ videoId: "5TN6l7JGWvs" });
