...
const ID = "mrscraper#booking/reviews";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = {
    body: { "url": "https://www.booking.com/hotel/th/admiral.html" },
};

Deno.test(`${ID} happy (synthetic): 36 tokens, the caller gets the inner data`, async () => {
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    const twin = await run({
        "url": "https://www.booking.com/hotel/th/admiral.en-gb.html",
    });
