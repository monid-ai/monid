...
const ID = "mrscraper#agoda/reviews";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = {
    body: {
        "url": "https://www.agoda.com/admiral-suites/hotel/bangkok-th.html",
    },
};

Deno.test(`${ID} happy (synthetic): 41 tokens, the caller gets the inner data`, async () => {
...
Deno.test(`${ID} no reviews (synthetic): a 200 with reviews [] records zero`, async () => {
    const unit = await testSealedUnit(ID);
    const fixture = await loadFixture(
        `${fixturesDir}synthetic-empty-reviews.json`,
    );
    const result = await runEndpoint({
        unit,
        input: INPUT,
    ...
    // the vendor reports 41 tokens; the claim is withheld and the fold is
    // zero (v1 hasNoReviews; design D12)
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    for (
        const bad of [
            {
                "url":
                    "https://www.booking.com/admiral-suites/hotel/bangkok-th.html",
            },
            {
                "url":
                    "https://agoda.attacker.example/admiral-suites/hotel/bangkok-th.html",
            },
            {
                "url":
    ...
    const twin = await run({
        "url": "https://www.agoda.co.th/admiral-suites/hotel/bangkok-th.html",
    });
