...
const ID = "mrscraper#watsons/category";
...
Deno.test(`${ID} happy (synthetic): 21 tokens, the caller gets the inner data`, async () => {
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    for (
        const bad of [
            { "url": "https://www.watsons.com.my/p/vitamin-c-1000mg/BP_123" },
            { "url": "https://watsons.attacker.example/health-care/c/110100" },
            {
                "url":
                    "https://www.guardian.com.my/health-care/vitamins-minerals/c/110100",
            },
        ]
    ) {
    ...
    const twin = await run({
        "url":
            "https://www.watsons.com.sg/health-care/vitamins-minerals/c/110100",
    });
