...
const ID = "mrscraper#1688/category";
...
Deno.test(`${ID} happy (synthetic): 29 tokens, the caller gets the inner data`, async () => {
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    for (
        const bad of [
            {
                "url":
                    "https://www.taobao.com/selloffer/offer_search.htm?keywords=phone",
            },
            {
                "url":
                    "https://1688.attacker.example/selloffer/offer_search.htm",
            },
        ]
    ) {
