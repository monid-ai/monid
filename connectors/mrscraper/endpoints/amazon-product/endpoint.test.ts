...
const ID = "mrscraper#amazon/product";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = {
    body: { "url": "https://www.amazon.com/dp/B0CP9YB3Q4" },
};

Deno.test(`${ID} happy (synthetic): 50 tokens, the caller gets the inner data`, async () => {
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    const twin = await run({
        "url": "https://www.amazon.co.uk/gp/product/B0CP9YB3Q4",
    });
