...
const ID = "mrscraper#trip/reviews";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = {
    body: { "url": "https://us.trip.com/hotels/shanghai-hotel-detail-992573/" },
};
...
Deno.test(`${ID} no reviews (synthetic): a 200 with reviews [] records zero`, async () => {
    ...
    // the vendor reports 31 tokens; the claim is withheld and the fold is
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    const twin = await run({
        "url": "https://www.trip.com/hotels/shanghai-hotel-detail-992573/",
    });
