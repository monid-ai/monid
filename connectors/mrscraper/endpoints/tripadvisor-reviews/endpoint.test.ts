...
const ID = "mrscraper#tripadvisor/reviews";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = {
    body: { "url": "https://www.tripadvisor.com/Hotel_Review-g293916-d308699" },
};
...
Deno.test(`${ID} no reviews (synthetic): a 200 with reviews [] records zero`, async () => {
    ...
    // the vendor reports 20 tokens; the claim is withheld and the fold is
...
Deno.test(`${ID}: only this site's URL passes the gate; the body is strict`, async () => {
    ...
    for (
        const bad of [
            { "url": "https://www.yelp.com/Hotel_Review-g293916-d308699" },
            {
                "url":
                    "https://tripadvisor.attacker.example/Hotel_Review-g293916-d308699",
            },
            {
                "url":
    ...
    const twin = await run({
        "url": "https://www.tripadvisor.co.uk/Hotel_Review-g293916-d308699",
    });
