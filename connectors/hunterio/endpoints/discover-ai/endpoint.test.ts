...
Deno.test(`${ID} happy (synthetic): the quota gate: 8.36 credits`, async () => {
...
Deno.test(`${ID}: the schema gate — the vendor's rules and strictness`, async () => {
    ...
    for (
        const bad of [
            {},
            { query: "   " },
            { query: "software", organization: { domain: ["hunter.io"] } },
            { query: "software", limit: 0 },
        ]
    ) {
