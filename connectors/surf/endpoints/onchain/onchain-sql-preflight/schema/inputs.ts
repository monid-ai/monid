...
/** POST /onchain/sql/preflight body (ported from v1; faithful vendor mirror,
 *  optionality only — vendor defaults are applied at the binding). */
export const zOnchainSqlPreflightBody = z.object({
    sql: z.string().min(1).describe(
        "Read-only ClickHouse SELECT/WITH query to validate and " +
            "estimate WITHOUT executing. Same shape and rules as " +
