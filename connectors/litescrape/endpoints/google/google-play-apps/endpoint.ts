...
import { zGooglePlayAppsQueryParams } from "./schema/inputs.ts";

/** GET /google/play/apps — Search Play Apps. */
export default defineEndpoint({
    meta: {
        displayName: "Search Play Apps",
        summary:
            "Search or browse Android apps, categories, device storefronts, " +
            "and charts on Google Play.",
        description:
            "Search Google Play for Android apps or browse its storefront. " +
            "Returns organic_results (title, product_id, developer, rating, " +
            "price, icon), app_highlight and items_highlight when the store " +
            "shows them, charts, and litescrape_pagination tokens. Supports a " +
            "query, a native category, a chart (top free, top paid, top " +
            "grossing), device storefronts (phone, tablet, TV, Chromebook, " +
            "watch, car), a children's age range, and continuation tokens. " +
            "For one app's details and download count pass its product_id to " +
            "google/play/product. Suited for app market research, competitor " +
            "tracking, and category rankings.",
        docsUrl: "https://litescrape.com/docs/google-play-apps",
        ...
        notes: [
            ...
            "`age` requires `apps_category` 'FAMILY'.",
        ],
    },
    request: { method: "GET", path: "/google/play/apps" },
    input: {
        schema: {
            queryParams: zGooglePlayAppsQueryParams,
        },
    },
    usage: {
        ...
        /** v1 RESULT_GROUPS["/google-play/apps"] through hasAnyResultGroup (design
