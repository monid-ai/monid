...
const ID = "vaquill#us/statutes/section/{act_id}/definitions";
const FIXTURES = fromFileUrl(new URL("../../fixtures/", import.meta.url));
const INPUT = { pathParams: { act_id: "USC_T42_C21_S1983" } };

Deno.test(`${ID} happy: one answered lookup, whatever the term count`, async () => {
    const unit = await testSealedUnit(ID);
    const result = await runEndpoint({
        unit,
        input: INPUT,
        mode: "replay",
        fixture: await loadFixture(`${FIXTURES}definitions-ok.json`),
    });
...
Deno.test(`${ID} provider error: a 401 is data, and bills nothing`, async () => {
    ...
    const result = await runEndpoint({
        ...
        mode: "replay",
        fixture: await loadFixture(`${FIXTURES}unauthorized-definitions.json`),
    });
...
Deno.test(`${ID} schema gate: an act_id shorter than the vendor's minimum is refused before the wire`, async () => {
    const unit = await testSealedUnit(ID);
    const fixture = await loadFixture(`${FIXTURES}definitions-ok.json`);
    ...
    // everything: three characters is accepted. Checked through the PURE estimate, so
    // no fixture and no wire call is involved.
    assertEquals(
        await estimateEndpoint(unit, { pathParams: { act_id: "abc" } }),
        { credits: { default: 4 }, evidence: { RESULT: 1 } },
    );
});
