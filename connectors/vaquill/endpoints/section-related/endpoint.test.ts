...
const ID = "vaquill#us/statutes/section/{act_id}/related";
...
Deno.test(`${ID} happy: flat 2 credits whatever the window`, async () => {
    const unit = await testSealedUnit(ID);
    const result = await runEndpoint({
        unit,
        input: INPUT,
        mode: "replay",
        fixture: await loadFixture(`${FIXTURES}related-ok.json`),
    });
...
Deno.test(`${ID} provider error: a 401 is data, and bills nothing`, async () => {
    ...
    const result = await runEndpoint({
        ...
        mode: "replay",
        fixture: await loadFixture(`${FIXTURES}unauthorized-related.json`),
    });
...
Deno.test(`${ID} schema gate: limit above the vendor's 10 per side is refused`, async () => {
    const unit = await testSealedUnit(ID);
    const fixture = await loadFixture(`${FIXTURES}related-ok.json`);
    ...
    // everything: 10 is accepted. Checked through the PURE estimate, so
