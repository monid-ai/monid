...
const ID = "contextdev#web/sic";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = { queryParams: { input: "stripe.com", type: "latest_sec" } };
...
Deno.test(`${ID}: the compiled schema gates the dataset and the code bounds`, async () => {
    const unit = await testSealedUnit(ID);
    const fixture = await loadFixture(`${fixturesDir}synthetic-happy.json`);
    ...
    for (
        const bad of [
            { input: "stripe.com", type: "naics" },
            { input: "stripe.com", minResults: 0 },
            { type: "latest_sec" },
        ]
    ) {
