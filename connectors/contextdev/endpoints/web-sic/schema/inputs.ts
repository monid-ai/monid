...
/** GET /web/sic query params — the vendor mirror
 *  (docs.context.dev/api-reference/web-extraction/sic, 2026-09-17), scalar
 *  params only (design D6). */
export const zSicQueryParams = z.object({
    ...
    type: z.enum(["original_sic", "latest_sec"]).describe(
        "Which SIC dataset to classify against: 'original_sic' (the 1987 " +
            "Standard Industrial Classification, default) or 'latest_sec' " +
            "(the current list published by the SEC).",
    ).optional(),
