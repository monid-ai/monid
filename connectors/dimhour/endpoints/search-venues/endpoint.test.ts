...
const ID = "dimhour#search-venues";
const TOOL = "search_venues";
const fixturesDir = fromFileUrl(new URL("./fixtures/", import.meta.url));
const INPUT = assertSmall({
    body: { city: "dallas", query: "ramen", limit: 2 },
});
...
Deno.test(`${ID} schema gate: the source's bounds, required fields and strictness reject before the wire`, async () => {
    ...
    const rejected: [Record<string, Json>, string][] = [
        ...
        [{ city: "dallas", min_score: 101 }, "min_score above 100"],
        [{ city: "dallas", min_score: -1 }, "min_score below 0"],
        [{ city: "dallas", sort: "random" }, "sort outside the enum"],
        [
            { city: "dallas", bulk: true },
            "an unknown field (the source is additionalProperties: false)",
        ],
    ];
...
Deno.test(`${ID} schema gate: the passing near-twins at each boundary (no IO)`, async () => {
    ...
    const accepted: [Record<string, Json>, string][] = [
        ...
        [{ city: "dallas", min_score: 100 }, "min_score at 100"],
        [{ city: "dallas", min_score: 0 }, "min_score at 0"],
        [{ city: "dallas", sort: "name_asc" }, "sort inside the enum"],
        [{}, "every field is optional (city omitted searches all cities)"],
    ];
